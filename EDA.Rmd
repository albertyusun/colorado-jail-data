...
colorado_num_percent <- colorado_num_inmates %>%
  ...
  mutate(jail_black_pct = black_1 / total_1) %>%
  mutate(jail_hispanic_pct = hispanic_1 / total_1) %>%
  select(county, difference, male_percent, black_percent, hispanic_percent, type)
...
We are 95% confident that a overall percent increase in male inmates in a jail will result in between a 0.5% to 1.1% decrease in jail population. 

We are 95% confident that a overall percent increase in Hispanic inmates in a jail will result in between a 0.1% to 0.7% decrease in jail population. 

We are 95% confident that an Urbanized Area will result in between a 7.190% to 8.052% decrease in jail population compared to Rural county. 

We are 95% confident that an Urbanized Cluster will result in between a 7.074% to 7.926% decrease in jail population compared to Rural county. 

### Not Percent model

```{r}
colorado_num <- colorado_num_inmates %>%
  mutate(difference = (total_3 - total_1)) %>%
  rename(male = male_1) %>%
  rename(white = white_1) %>%
  rename(black = black_1) %>%
  rename(native = native_american_1) %>%
  rename(other_race = other_race_1) %>%
  rename(non_hispanic = non_hispanic_1) %>%
  rename(hispanic = hispanic_1) %>%
  rename(unknown_ethnicity = unknown_ethnicity_1) %>%
  select(county, difference, male, black, hispanic)

colorado_num
```

```{r merge}
colorado_num <- inner_join(colorado_num, pop, by = c("county" = "county")) %>%
  mutate(type = if_else(population < 2500, "R", 
                        if_else(population < 50000, "UC", "UA")))
```

Urban or rural: https://www.census.gov/programs-surveys/geography/guidance/geo-areas/urban-rural.html
...
int_only_model <- lm(difference ~ 1, data = colorado_num)

covid_model1 <- step(full_model, scope = formula(int_only_model), direction = "backward")
...
There is a statistically significant relationship between releases and males, releases and black population, releases and hispanic populations.
