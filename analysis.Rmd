...
```{r}
colorado_num_inmates <- inner_join(colorado_num_inmates, pop, by = c("county" = "County")) %>%
...
sample_data <- sample_n(colorado_num_percent, 25)
```
...
int_only_model <- lm(difference ~ 1, data = sample_data)
...
colorado_num <- colorado_num_inmates %>%
  ...
  mutate(male = male_1) %>%
  mutate(white = white_1) %>%
  mutate(black = black_1) %>%
  mutate(native = native_american_1) %>%
  mutate(other_race = other_race_1) %>%
  mutate(non_hispanic = non_hispanic_1) %>%
  mutate(hispanic = hispanic_1) %>%
  mutate(unknown_ethnicity = unknown_ethnicity_1) %>%
